...
pub fn definitely_multi_line<F: FnOnce(Writer) -> Option<Writer>>(
    f: F,
...
pub fn single_line<F: FnOnce(Writer) -> Option<Writer>>(
    f: F,
) -> impl FnOnce(Writer) -> Option<Writer> {
    move |i| i.with_single_line(f)
}

pub fn indented<F: FnOnce(Writer) -> Option<Writer>>(
    f: F,
) -> impl FnOnce(Writer) -> Option<Writer> {
